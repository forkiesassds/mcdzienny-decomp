...
namespace MCDzienny
{
    ...
    {
        const string SETTINGSROOT = "GeneralSettings";

        readonly string fileName = "properties/general.properties";
            ...
            get
            {
                ...
                if (m_SettingsXML == null)
                {
                    ...
                    catch
                    {
                        ...
                        XmlNode val2 = m_SettingsXML.CreateNode((XmlNodeType)1, "GeneralSettings", "");
        ...
        string GetValue(SettingsProperty setting)
        {
            ...
            try
            {
                ...
                {
                    return SettingsXML.SelectSingleNode("GeneralSettings/" + setting.Name).InnerText;
                }
                return SettingsXML.SelectSingleNode("GeneralSettings/" + setting.Name).InnerText;
            }
            catch
        ...
        void SetValue(SettingsPropertyValue propVal)
        {
            ...
            try
            {
                val = !IsRoaming(propVal.Property) ? (XmlElement)SettingsXML.SelectSingleNode("GeneralSettings/" + propVal.Name)
                    : (XmlElement)SettingsXML.SelectSingleNode("GeneralSettings/" + propVal.Name);
            }
            catch (Exception)
            {
                val = null;
            ...
            else if (IsRoaming(propVal.Property))
            {
                ...
                val.InnerText = propVal.SerializedValue.ToString();
                SettingsXML.SelectSingleNode("GeneralSettings").AppendChild(val);
            }
            else
            {
                val = SettingsXML.CreateElement(propVal.Name);
