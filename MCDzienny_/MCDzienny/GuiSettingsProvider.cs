...
namespace MCDzienny
{
    ...
    {
        const string SETTINGSROOT = "GuiSettings";

        readonly string fileName = "properties/gui.properties";
            ...
            get
            {
                ...
                if (m_SettingsXML == null)
                {
                    ...
                    catch (Exception)
                    {
                        ...
                        XmlNode val2 = m_SettingsXML.CreateNode((XmlNodeType)1, "GuiSettings", "");
        ...
        string GetValue(SettingsProperty setting)
        {
            ...
            try
            {
                ...
                {
                    return SettingsXML.SelectSingleNode("GuiSettings/" + setting.Name).InnerText;
                }
                return SettingsXML.SelectSingleNode("GuiSettings/" + setting.Name).InnerText;
            }
            catch (Exception)
        ...
        void SetValue(SettingsPropertyValue propVal)
        {
            ...
            try
            {
                val = !IsRoaming(propVal.Property) ? (XmlElement)SettingsXML.SelectSingleNode("GuiSettings/" + propVal.Name)
                    : (XmlElement)SettingsXML.SelectSingleNode("GuiSettings/" + propVal.Name);
            }
            catch (Exception)
            {
                val = null;
            ...
            else if (IsRoaming(propVal.Property))
            {
                ...
                val.InnerText = propVal.SerializedValue.ToString();
                SettingsXML.SelectSingleNode("GuiSettings").AppendChild(val);
            }
            else
            {
                val = SettingsXML.CreateElement(propVal.Name);
