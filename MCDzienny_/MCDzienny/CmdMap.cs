        ...
        public override void Use(Player p, string message)
        {
            ...
            if (message.IndexOf(' ') == -1)
            {
                ...
                if (level != null)
                {
                    ...
                    Player.SendMessage(p, string.Format("Survival death: {0}(Fall: {1}, Drown: {2})", FoundCheck(level.Death), level.fall, level.drown));
            ...
            string text = message.IndexOf(' ') != -1 ? message.Split(' ')[0].ToLower() : message.ToLower();
            try
            {
                switch (text)
                {
